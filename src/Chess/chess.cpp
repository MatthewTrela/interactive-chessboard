...
Bitboard getQueenAttacks(Square sq, Bitboard occ) { return getRookAttacks(sq, occ) | getBishopAttacks(sq, occ); }
}  // namespace Attacks
...
IRAM_ATTR void Board::updateOccupancy() {
    colors[static_cast<int>(ChessColor::White)] =
        pieces[0][0] | pieces[0][1] | pieces[0][2] | pieces[0][3] | pieces[0][4] | pieces[0][5];
    colors[static_cast<int>(ChessColor::Black)] =
        pieces[1][0] | pieces[1][1] | pieces[1][2] | pieces[1][3] | pieces[1][4] | pieces[1][5];
    ...
    for (char c : token) {
        if (c == '/') {
            ...
            switch (std::tolower(c)) {
                case 'p':
                    type = static_cast<int>(PieceType::Pawn);
                    break;
                case 'n':
                    type = static_cast<int>(PieceType::Knight);
                    break;
                case 'b':
                    type = static_cast<int>(PieceType::Bishop);
                    break;
                case 'r':
                    type = static_cast<int>(PieceType::Rook);
                    break;
                case 'q':
                    type = static_cast<int>(PieceType::Queen);
                    break;
                case 'k':
...
bool Board::isCheck() const {
    ...
    return isSquareAttacked(BitUtils::getLSB(pieces[turn][static_cast<int>(PieceType::King)]),
                            static_cast<ChessColor>(turn ^ 1));
}
...
IRAM_ATTR void Board::generatePseudoLegalMoves(MoveList& moveList) const {
    ...
    if (sideToMove == ChessColor::White) {
        ...
        if (castlingRights & 1) {
            ...
            if ((occupancy & pathMask) == 0) {
                if (!isSquareAttacked(SQ_E1, ChessColor::Black) && !isSquareAttacked(SQ_F1, ChessColor::Black) &&
        ...
        if (castlingRights & 2) {
            ...
            if ((occupancy & pathMask) == 0) {
                if (!isSquareAttacked(SQ_E1, ChessColor::Black) && !isSquareAttacked(SQ_D1, ChessColor::Black) &&
        ...
        if (castlingRights & 4) {
            ...
            if ((occupancy & pathMask) == 0) {
                if (!isSquareAttacked(SQ_E8, ChessColor::White) && !isSquareAttacked(SQ_F8, ChessColor::White) &&
        ...
        if (castlingRights & 8) {
            ...
            if ((occupancy & pathMask) == 0) {
                if (!isSquareAttacked(SQ_E8, ChessColor::White) && !isSquareAttacked(SQ_D8, ChessColor::White) &&
    ...
    Bitboard diagonalSliders =
        pieces[us][static_cast<int>(PieceType::Bishop)] | pieces[us][static_cast<int>(PieceType::Queen)];
    ...
    Bitboard straightSliders =
        pieces[us][static_cast<int>(PieceType::Rook)] | pieces[us][static_cast<int>(PieceType::Queen)];
...
uint64_t Board::perft(int depth) {
    ...
    if (depth == 1) {
        uint64_t nodes = 0;
        for (size_t i = 0; i < pseudoMoves.size(); ++i)
            if (makeMove(pseudoMoves[i])) {
                ++nodes;
                unmakeMove(pseudoMoves[i]);
            }
        return nodes;
    }
...
Bitboard Board::getOccupancy() { return occupancy; }
